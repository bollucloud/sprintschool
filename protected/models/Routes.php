...
class Routes extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('cost, create_user_id, update_user_id', 'numerical', 'integerOnly'=>true),
			array('destination', 'length', 'max'=>255),
			array('create_time, update_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, destination, cost, create_time, create_user_id, update_time, update_user_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'vehicles' => array(self::HAS_MANY, 'Vehicles', 'route_id'),
		);
	}
	...
	public static function model($className=__CLASS__)
	{
        ...
        public static function getRouteOptions(){
            $list = CHtml::listData(Routes::model()->findAll(array('order' => 'destination')), 'id', 'destination');
            return $list;
        }
}
